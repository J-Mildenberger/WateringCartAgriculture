...
void MyQueue_Init(tQueue *q, void *buffer, uint32_t elementSize,
		uint32_t capacity, const char *pName)
{
...
bool MyQueue_Enqueue(tQueue *q, const void *element)
{
	...
	if (!element)
	{
		DBG_PRINT("ERROR: NULL element passed to queue %s", q->name);
		return false;
	}
...
bool MyQueue_Dequeue(tQueue *q, void *element)
{
	...
	uint8_t *src = q->buffer + (q->tail * q->elementSize); //#findme dequeue causing HaurdFault!!
